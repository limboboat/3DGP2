...
CCamera *CPlayer::OnChangeCamera(DWORD nNewCameraMode, DWORD nCurrentCameraMode)
{
	...
	{
		//XMFLOAT3 targetPosition = XMFLOAT3(m_xmf3Position.x, m_xmf3Position.y, m_xmf3Position.z);
...
CCamera *CAirplanePlayer::ChangeCamera(DWORD nNewCameraMode, float fTimeElapsed)
{
	...
	switch (nNewCameraMode)
	{
		...
		case THIRD_PERSON_CAMERA:
			SetFriction(2.5f);	//¸¶Âû
			SetGravity(XMFLOAT3(0.0f, 0.0f, 0.0f));	//Áß·Â
			SetMaxVelocityXZ(40.5f);	//¼Óµµ
			SetMaxVelocityY(40.0f);
			m_pCamera = OnChangeCamera(THIRD_PERSON_CAMERA, nCurrentCameraMode);
			m_pCamera->SetTimeLag(0.0f);
			//m_pCamera->SetOffset(XMFLOAT3(0.0f, 0.0f, -5.0f));
			m_pCamera->SetOffset(XMFLOAT3(0.0f, 0.f, -0.5f));//1ÀÎÄª 
			//m_pCamera->SetOffset(XMFLOAT3(0.0f, 15.0f, -30.0f)); //3ÀÎÄª
			//m_pCamera->SetPosition(Vector3::Add(m_xmf3Position, m_pCamera->GetOffset()));
			m_pCamera->SetPosition(Vector3::Add(XMFLOAT3(
				m_xmf3Position.x, 
				m_xmf3Position.y, 
				m_xmf3Position.z+5.0f), m_pCamera->GetOffset()));

			m_pCamera->GenerateProjectionMatrix(1.01f, 5000.0f, ASPECT_RATIO, 20.0f);
			m_pCamera->SetViewport(0, 0, FRAME_BUFFER_WIDTH, FRAME_BUFFER_HEIGHT, 0.0f, 1.0f);
			m_pCamera->SetScissorRect(0, 0, FRAME_BUFFER_WIDTH, FRAME_BUFFER_HEIGHT);
			break;
		default:
